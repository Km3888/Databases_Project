...
def spending():
    ...
    default_month1=Purchase.query.join(Ticket, Purchase.ticket_id==Ticket.ticket_id).with_entities(func.sum(Ticket.price).label('all_sum')).filter(Purchase.email_customer==current_user.get_id().split('_')[1:]).filter(Purchase.date>=default_list_start_months[0]).filter(Purchase.date<default_list_start_months[1]).first()
    default_month1_sum=default_month1.all_sum

    default_month2=Purchase.query.join(Ticket, Purchase.ticket_id==Ticket.ticket_id).with_entities(func.sum(Ticket.price).label('all_sum')).filter(Purchase.email_customer==current_user.get_id().split('_')[1:]).filter(Purchase.date>=default_list_start_months[1]).filter(Purchase.date<default_list_start_months[2]).first()
    default_month2_sum=default_month2.all_sum

    default_month3=Purchase.query.join(Ticket, Purchase.ticket_id==Ticket.ticket_id).with_entities(func.sum(Ticket.price).label('all_sum')).filter(Purchase.email_customer==current_user.get_id().split('_')[1:]).filter(Purchase.date>=default_list_start_months[2]).filter(Purchase.date<default_list_start_months[3]).first()
    default_month3_sum=default_month3.all_sum

    default_month4=Purchase.query.join(Ticket, Purchase.ticket_id==Ticket.ticket_id).with_entities(func.sum(Ticket.price).label('all_sum')).filter(Purchase.email_customer==current_user.get_id().split('_')[1:]).filter(Purchase.date>=default_list_start_months[3]).filter(Purchase.date<default_list_start_months[4]).first()
    default_month4_sum=default_month4.all_sum

    default_month5=Purchase.query.join(Ticket, Purchase.ticket_id==Ticket.ticket_id).with_entities(func.sum(Ticket.price).label('all_sum')).filter(Purchase.email_customer==current_user.get_id().split('_')[1:]).filter(Purchase.date>=default_list_start_months[4]).filter(Purchase.date<default_list_start_months[5]).first()
    default_month5_sum=default_month5.all_sum

    default_month6=Purchase.query.join(Ticket, Purchase.ticket_id==Ticket.ticket_id).with_entities(func.sum(Ticket.price).label('all_sum')).filter(Purchase.email_customer==current_user.get_id().split('_')[1:]).filter(Purchase.date>=default_list_start_months[5]).first()
    default_month6_sum=default_month6.all_sum

    # str(one_year_ago) = '2018-12-02 01:09:18.691390'
    # same format for datetime.now() = '2019-12-02 01:11:50.958864'
    # invalid: timedelta(days=02)
...
#users.query.join(friendships, users.id==friendships.user_id).add_columns(users.userId, users.name, users.email, friends.userId, friendId).filter(users.id == friendships.friend_id).filter(friendships.user_id == userID).paginate(page, 1, False)
    return render_template('customer/spending.html',default_total_sum=default_total_sum,\
                            ...
                            default_month1_sum=default_month1_sum,\
                            default_month2_sum=default_month2_sum,\
                            default_month3_sum=default_month3_sum,\
                            default_month4_sum=default_month4_sum,\
                            default_month5_sum=default_month5_sum,\
                            default_month6_sum=default_month6_sum,\
