...
def agent():
    ...
    if form.validate_on_submit():
        agent=Airline_Staff.query.filter_by(email=form.email.data.lower()).first()
        if agent is not None and agent.verify_password(form.password.data) and agent.verify_id(form.id.data):
            login_user(agent)
            next=request.args.get('next')
            if next is None or not next.startwith('/'):
                next=url_for(next)
            return redirect(next)
        flash('Invaliid email, password, or ID')
    return render_template('auth/login.html',form=form)
